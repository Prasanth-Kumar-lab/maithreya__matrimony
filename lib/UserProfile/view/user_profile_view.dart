        ...
        children: [
          Stack(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () {
                  final imageProvider = controller.getBackgroundImage();
                  ...
                  child: Obx(
                      ...
                      icon: controller.isUpdating.value
                        ...
                        child: CircularProgressIndicator(
                          ...
                          : const Icon(
                        Icons.camera_alt,
                        color: AppColors.profileIconsColor,
                        size: 20,
                      ),
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    IconButton(
                      icon: const Icon(Icons.add_a_photo_rounded, color: AppColors.profileIconsColor),
                      onPressed: controller.pickMultipleImages,
                      tooltip: "Add from Gallery",
                    ),
                    IconButton(
                      icon: const Icon(Icons.camera, color: AppColors.profileIconsColor),
                    ...
                    if (hasNewPhotos) ...[
                      ...
                      IconButton(
                        icon: Icon(Icons.close, color: AppColors.profileIconsColor),
                        onPressed: () {
                      ...
                      IconButton(
                        icon: controller.isUpdating.value
                          ...
                          child: CircularProgressIndicator(
                            ...
                            : Icon(Icons.save, color: AppColors.profileIconsColor),
                        /*onPressed:
                        ...
                        onPressed: controller.isUpdating.value
                          ...
                          if (!controller.isUpdating.value) {
                            controller.exitEditMode("Photos");
                          }
                        },
                      ),
                    ],
                  ...
                  Positioned(
                    ...
                    child: GestureDetector(
                      ...
                      child:CircleAvatar(
                        ...
                        child: Icon(Icons.close, size: 16, color: AppColors.profileIconsColor),
                      ),
                    ),
                  ),
                ],
          ...
          if (allPhotos.isEmpty && controller.selectedPhotos.isEmpty)
            const Center(
              child: Text("No photos yet", style: TextStyle(color: AppColors.textColor)),
            )
          else
            SizedBox(
              height: 110,
            ...
            Row(
              ...
              children: [
                Expanded(
                  child: Text(
                    title,
                    style: const TextStyle(fontSize: 18, fontWeight: AppColors.fontWeight, color: AppColors.textColor),
                  ),
                ),
                if (!isEditing)
                  IconButton(
                    icon: const Icon(Icons.edit, color: AppColors.profileIconsColor),
                  ...
                  Row(
                    children: [
                      IconButton(
                        icon: const Icon(Icons.cancel, color: AppColors.profileIconsColor),
                      ...
                      Obx(() => IconButton(
                        icon: controller.isUpdating.value
                          ...
                          child: CircularProgressIndicator(color: Colors.green, strokeWidth: 2),
                            ...
                            : const Icon(Icons.save, color: AppColors.profileIconsColor),
                        onPressed: controller.isUpdating.value
                            ? null
                            : () async {
  ...
  Widget _buildCasteInfoSection(MyProfileController controller) {
    return _buildSectionCard(
      ...
      viewContent: _buildViewContent(controller, [
        ...
        _buildInfoRow("Religion", controller.user.religion, Icons.person),
      ]),
      editContent: _buildEditContent(controller, [
        ...
        _buildEditableField("Religion", controller.religionController, icon: Icons.person),
      ]),
      onSave: controller.saveBasicInfo,
    );
  }
...
class MyProfileScreen extends StatelessWidget {
  final UserProfile user;
  const MyProfileScreen({Key? key, required this.user}) : super(key: key);
  Future<void> _onRefresh(MyProfileController controller) async {
    // Fetch the latest profile from server using userId
    try {
      final freshProfile = await controller.fetchMyProfile(controller.user.userId);

      if (freshProfile != null) {
        // Update the existing controller with fresh data
        controller.user = freshProfile;

        // Re-initialize all text controllers with new values
        controller.initControllers();

        // Update server photos list
        controller.serverPhotos.assignAll(freshProfile.photosList ?? []);
