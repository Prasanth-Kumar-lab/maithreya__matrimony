...
class UsersProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Dynamically retrieve currentUserId from homeController
    String currentUserId = homeController.currentUserId ?? '';

    // Register MatchesController if not already registered
    if (!Get.isRegistered<MatchesController>()) {
      Get.put(MatchesController(currentUserId));
    }
    ...
    return Scaffold(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                _buildCardSection("Community", [
                  _buildInfoRow("Caste", user.caste,
                      icon: Icons.home_outlined),
                  _buildInfoRow("Sub-caste", user.subCaste, icon: Icons.map_outlined),
                  _buildInfoRow("Religion", user.religion, icon: Icons.location_city),
                ]),
      ...
      floatingActionButton: Obx(() {
        final isLiked = matchesController.likedProfiles
            .any((p) => p['id'].toString() == user.userId.toString());
        return Row(
          ...
          children: [
            ...
            const SizedBox(width: 12),
            // ── FAVOURITE (with Lottie + dynamic background) ── only if not liked
            if (!isLiked)
              Builder(
                builder: (context) {
                    ...
                    builder: (context, setState) {
                      return _buildFAB(
                        ...
                        onTap: () async {
                          ...
                          // Refresh the liked profiles list
                          await matchesController.fetchLikedProfiles();
                        },
...
class UsersProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Dynamically retrieve current logged-in userId
    String currentUserId = homeController.currentUserId ?? '';
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                        ...
                        onTap: () {
                          if (user.image != null && user.image!.isNotEmpty) {
                            showDialog(
                              ...
                              barrierColor: Colors.black.withOpacity(0.7), // semi-transparent bg
                              builder: (context) {
                                return Dialog(
                ...
                _buildCardSection("Community", [
                  _buildInfoRow("Caste", user.caste, icon: Icons.home_outlined),
                  _buildInfoRow("Sub-caste", user.subCaste,
  ...
  // Photos Section (FIXED)
  // ──────────────────────────────
  Widget _buildPhotosSection(UserProfile user) {
    final photos = user.photosList ?? [];
