...
class _CompleteProfileScreenState extends State<CompleteProfileScreen>
  ...
  void initState() {
    ...
    // Fetch profile data (no caste fetch)
    _fetchProfileData();
  }
  // ──────────────────────────────────────
  // Image helpers
    ...
    try {
      ...
      if (profile != null) {
        ...
        if (profile.hobbies != null &&
            !['Reading', 'Traveling', 'Music', 'Sports', 'Cooking', 'Dancing']
                .contains(profile.hobbies!) &&
        ...
        interestsController.text = profile.otherInterests ?? '';
        if (profile.otherInterests != null &&
            !['Fitness', 'Movies', 'Yoga', 'Gaming', 'Art']
                .contains(profile.otherInterests!) &&
            profile.otherInterests!.isNotEmpty) {
  ...
  Future<bool> _uploadZodiacData() async {
    if (selectedZodiacSign == null) return false;
    try {
      var request = http.MultipartRequest(
        'POST',
        Uri.parse(ApiEndPoint.profileApiEndPoint),
      ...
      debugPrint('Zodiac upload error: $e');
      return false;
    }
  }
  ...
  Future<void> _onContinue() async {
    if (currentStep == 1) {
      ...
      if (selectedZodiacSign == null) {
        _showSnack('Please select your zodiac sign');
        return;
      }
      _showLoading();
      ...
      // Lifestyle
      if (hobbiesController.text.trim().isEmpty ||
          ...
          Column(
            ...
            children: [
              ...
              GridView.count(
                ...
                childAspectRatio: 3,
                children:
                ['2L+', '3L+', '5L+', '7L+', '10L+', 'Other'].map((income) {
            ...
            children: [
              ...
              GridView.count(
                ...
                children: [
                  ...
                  final isSelected = hobbiesController.text == hobby ||
            ...
            children: [
              ...
              GridView.count(
                ...
                children: [
                  ...
                  final isSelected = interestsController.text == interest ||
                    ...
                    onTap: () {
                      setState(() {
                        if (interest == 'Other') {
                          ...
                          interestsController.text = interest;
                        }
                      });
                    },
                  );
...
class _CompleteProfileScreenState extends State<CompleteProfileScreen>
  ...
  Future<void> _fetchCastes() async {
    try {
      ...
      if (json is List) {
        list = json.cast<Map<String, dynamic>>();
      } else if (json is Map<String, dynamic>) {
        final data = json['castes'] ?? json['data'] ?? json['result'];
        if (data is List) list = data.cast<Map<String, dynamic>>();
      }
  ...
  Future<void> _fetchSubCastes(String casteName) async {
    try {
      final uri = Uri.parse(
        '${ApiEndPoint.getSubCasteApiEndPoint}?name=$casteName',
      );
      ...
      if (json is List) {
        ...
        final data = json['subcastes'] ?? json['data'] ?? json['result'];
        if (data is List) list = data.cast<Map<String, dynamic>>();
      }
      ...
      if (mounted) {
        setState(() {
          _subCasteList.clear();
          _subCasteList.addAll(list);
        });
      }
    } catch (e) {
      debugPrint('fetch sub-castes error: $e');
    }
  }
    ...
    try {
      ...
      if (profile != null && mounted) {
        ...
        if (selectedCaste != null && selectedCaste!.isNotEmpty) {
          await _fetchSubCastes(selectedCaste!);
        }
      }
    } catch (e) {
      debugPrint('Error fetching profile: $e');
